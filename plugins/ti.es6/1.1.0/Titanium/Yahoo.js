...
export default class Yahoo {
	...
	constructor (object) {
		if (object && object.apiName && object.apiName.endsWith('.Yahoo')) {
			this._object = object;
		} else {
	...
	static get bubbleParent () {
		return Titanium.Yahoo.bubbleParent;
	}
	static set bubbleParent (value) {
		Titanium.Yahoo.bubbleParent = value;
	}
	...
	yql (yql, callback) {
		if (!callback) {
			return new Promise(resolve => this._object.yql(yql, resolve));
		}
		return this._object.yql(yql, callback);
	}
	getApiName () {
